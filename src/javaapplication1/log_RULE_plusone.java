...
public class log_RULE_plusone {
    ...
    public log_RULE_plusone(Inicializacia start) {
        this.start = start;
    ...
    public void lepidlo(int num) {
        ...
        for (int i = 1; i < start.p_stlpcov; i++) {//bod 0
            if ((riesenie.get(num).get(i - 1).value == 1 || riesenie.get(num).get(i - 1).value == 3) && riesenie.get(num).get(i).value == 1) {
        ...
        for (int i = 0; i < pocet; i++) {
            ...
            if (i == 0 || start.pole_hodnot[num][i - 1][1] < zac) {//pravidlo robime, len ak sa cisla neprekryvaju 
                ...
                for (int y = zac; y <= kon; y++) {//case 2
                    ...
                    if (riesenie.get(num).get(y).value == 0 && bola_cierna == true) {
